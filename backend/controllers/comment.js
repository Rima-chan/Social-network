...
exports.createNewComment = async (req, res) => {
    ...
    Comment.create([newComment.userId, newComment.publicationId, newComment.content, newComment.attachment])
        .then(() => {
            ...
            Publication.addComment(nbComments)
                .then(() => res.status(200).json({message: 'Comment successfully created'}))
                .catch(err => res.status(500).json({error: 'Cannot add nb of comments in publication'}))
        })
        .catch(err => {
            console.log(err);
...
exports.updateComment =  async (req, res) => {
    ...
    if (attachment && comment[0].image_url) {
        const filename = comment[0].image_url.split('/images/')[1];
        fs.unlink(`images/${filename}`, (error) => {
            if (error) throw error;
        })
    } else {
        attachment = comment[0].image_url ? comment[0].image_url : null;
    }
...
exports.deleteComment = async (req, res) => {
    ...
    } else if (userId === comment[0].user_id || isAdmin) {
        Comment.deleteComment(commentId)
            .then(() => res.status(200).json({message: 'Comment sucessfully deleted !'}))
            .catch(err => {
                console.log(err);
